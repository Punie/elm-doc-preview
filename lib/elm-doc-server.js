...
function buildApplicationDocs(manifest, dir, elm) {
    ...
    if (manifest["source-directories"]) {
        manifest["source-directories"].forEach(function (src) {
            ...
            var elmJsonPath = path_1.default.resolve(dir, "elm.json");
            if (fs_1.default.existsSync(elmJsonPath)) {
                try {
                    var srcManifest = getManifestSync(elmJsonPath);
                    if (srcManifest) {
                        var srcModules = getExposedModules(tmpDir.name, srcDir, srcManifest);
...
function getExposedModules(dstDir, srcDir, manifest) {
    var exposed_modules = [];
    if (manifest["exposed-modules"]) {
        if (Array.isArray(manifest["exposed-modules"])) {
            ...
            Object.values(manifest["exposed-modules"]).forEach(function (modules) {
